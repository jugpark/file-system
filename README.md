...
**고도화 2차(2026-07-20, 스펙 외 신규)**:

- **파일 요청 링크** — 공유 링크의 역방향. 폴더 우클릭 → '파일 요청 링크'로 만들면
  외부인이 로그인 없이 그 폴더로 파일을 보낼 수 있는 페이지(`/share-upload/:token`)가
  생긴다. 기존 파일은 보이지 않고 내부 경로도 노출 안 됨, 같은 이름은 " (1)" 자동 회피,
  절대 덮어쓰기 없음, 전부 감사 기록(actor=share-link). 만료 1/7/30일, /shares에서
  다운로드/파일 요청 종류 구분 관리
- **폴더 구독 알림** — 폴더 우클릭 → '알림 구독'. 구독 폴더 아래 업로드/삭제가 생기면
  구독자에게 **Discord DM**(봇 토큰 필요, 본인 행동 제외, 5분 디바운스). 사라진 폴더의
  구독은 자동 정리

**고도화 3차(2026-07-20, 스펙 외 신규)**:

- **업로드 바이러스 스캔** — `.env`에 `CLAMAV_HOST=clamav`(compose에 ClamAV 서비스 포함)를
  넣으면 업로드를 clamd(INSTREAM, 무의존 TCP)로 검사. 감염 파일은 422로 거부. 파일 요청
  링크(무인증)는 스캐너 장애 시 **차단(fail-closed)**, 인증 업로드는 통과(fail-open).
  미설정이면 스캔 자체가 비활성이라 dev·미설치 환경에서 무해
- **오피스/한글 문서 미리보기** — docx/xlsx/pptx/hwpx를 더블클릭하면 다운로드 대신
  추출 텍스트를 모달에 표시(`GET /api/fs/preview-text`, 검색용 추출 캐시 우선·없으면 즉석).
  서식·이미지는 원본 다운로드 안내
- **PDF 첫 페이지 썸네일** — 그리드 뷰에서 PDF가 아이콘 대신 표지로. unpdf+@napi-rs/canvas로
  렌더 후 sharp webp 캐시(이미지 썸네일과 동일 경로). canvas 모듈이 없으면 아이콘으로 폴백
- **미리보기 좌우 이동** — 미리보기 모달에서 방향키(←/→)·좌우 버튼으로 같은 폴더의
  미리보기 가능한 파일들을 넘겨봄 (N/M 위치 표시)

**고도화 4차 — 열린 고리 닫기(2026-07-21, 스펙 외 신규)**:

- **접근 요청** — 못 보는 폴더(403)에서 '접근 요청' → 권한·사유 신청 → admin에게 웹훅
  알림. /admin '접근 요청' 큐에서 승인/반려(요청자에게 DM). ⚠ACL은 role 기반이라
  자동 부여 안 함 — 승인 후 admin이 ACL 규칙을 수동 추가(큐는 알림·처리 고리를 닫는 것)
- **세션 관리** — 프로필 → '세션 관리'에서 로그인 기기 목록(브라우저·OS·마지막 활동),
  낯선 기기 원격 종료, '다른 기기 전체 로그아웃'. 세션에 User-Agent·last_seen 기록
- **주간 활동 다이제스트** — 웹훅 설정 시 지정 요일·시각(`DIGEST_DAY`/`DIGEST_HOUR`,
  기본 월요일 9시)에 지난 7일 업로드/삭제/변경 수 + 상위 기여자를 채널로 1회 전송
  (구독 안 해도 흐름 파악). `DIGEST_DAY=-1`로 끔
- **백업 가시성** — `deploy/backup.sh`가 DB 옆에 `backup-status.json`(성공/실패·시각·크기)을
  남기고, /admin '백업 상태'에서 마지막 백업 결과 확인
